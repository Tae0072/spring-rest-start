...
public class AuthService {
    ...
    public void 로그인(LoginDTO reqDTO) {
        ...
        User findUser = userRepository.findByUsername(reqDTO.getUsername()) // findUser는 UserDetail타입으로 변함
                .orElseThrow(() -> new Exception401("유저네임을 찾을 수 없어요."));

        // 2. password를 hash해서 비교 검증
        boolean isSamePassword = bCryptPasswordEncoder.matches(reqDTO.getPassword(), findUser.getPassword());// reqDto.getPassword()
                                                                                                             // = 1234,
